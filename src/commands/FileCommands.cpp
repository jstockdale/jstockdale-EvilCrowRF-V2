 ...
 * @brief File command handler implementations.
 *
 * Extracted from FileCommands.h to reduce header bloat and
...
void FileCommands::registerCommands(CommandHandler& handler) {
 ...
 */
fs::FS& FileCommands::getFS(uint8_t pathType) {
 ...
 */
bool FileCommands::bufferedFileCopy(File& src, File& dst) {
    uint8_t buf[512];
    while (src.available()) {
 ...
 */
void FileCommands::buildBasePath(uint8_t pathType, PathBuffer& buffer) {
    buffer.clear();
 ...
 * @param buffer buffer to receive result
 */
void FileCommands::buildFullPath(uint8_t pathType, const char* relativePath, size_t pathLen, PathBuffer& buffer) {
 ...
 */
void FileCommands::extractFilename(const char* fullPath, PathBuffer& filename) {
    filename.clear();
...
// Binary tree builder
void FileCommands::buildDirectoryTreeBinaryRecursive(const char* path, uint8_t* buffer, size_t& offset, uint16_t& count, size_t maxBufferSize) {
...
//   5 = unknown error
bool FileCommands::handleGetFilesList(const uint8_t* data, size_t len) {
...
void FileCommands::sendBinaryFileListError(uint8_t errorCode) {
    size_t pathLen = strlen(pathBuffer.c_str());
...
void FileCommands::sendBinaryFileActionResult(uint8_t action, bool success, uint8_t errorCode, const char* path) {
...
// Load file data
bool FileCommands::handleLoadFileData(const uint8_t* data, size_t len) {
    if (len < 2) {
        return false;
    }
 ...
 */
bool FileCommands::removeDirectoryRecursive(fs::FS& fs, const char* path) {
...
bool FileCommands::handleRemoveFile(const uint8_t* data, size_t len) {
    if (len < 2) {
 ...
 */
bool FileCommands::handleFormatSDCard(const uint8_t* data, size_t len) {
...
bool FileCommands::handleRenameFile(const uint8_t* data, size_t len) {
    if (len < 3) {
...
bool FileCommands::handleCreateDirectory(const uint8_t* data, size_t len) {
    if (len < 2) {
...
bool FileCommands::handleSaveToSignalsWithName(const uint8_t* data, size_t len) {
    if (len < 3) {
...
// Copy file
bool FileCommands::handleCopyFile(const uint8_t* data, size_t len) {
    if (len < 3) {
        sendBinaryFileActionResult(4, false, 1); // 4=copy, error 1=insufficient data
        return false;
    }
...
bool FileCommands::handleMoveFile(const uint8_t* data, size_t len) {
    if (len < 4) {
...
void FileCommands::collectDirectoryPaths(const char* basePath, std::vector<String>& paths) {
    char fatfsPath[256];
...
bool FileCommands::handleGetDirectoryTree(const uint8_t* data, size_t len) {
    if (len < 1) {
...
void FileCommands::sendBinaryDirectoryTreeError(uint8_t errorCode) {
...
bool FileCommands::handleUploadFile(const uint8_t* data, size_t len) {
